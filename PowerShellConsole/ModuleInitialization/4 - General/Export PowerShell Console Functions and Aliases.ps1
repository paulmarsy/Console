...
Get-ChildItem (Join-Path $ProfileConfig.ConsolePaths.PowerShell "Exports\Functions") -Filter *.ps1 -Recurse | Sort-Object -Property FullName | % { . $_.FullName }
...
Get-ChildItem (Join-Path $ProfileConfig.ConsolePaths.PowerShell "Exports\Aliases") -Filter *.ps1 -Recurse | Sort-Object -Property FullName | % { . $_.FullName }
