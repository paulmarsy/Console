...
if ((Test-Path $includeFile) -and -not ([String]::IsNullOrWhiteSpace([IO.File]::ReadAllText($includeFile)))) {
    ...
    $includedFunctions | ? { $_ -notlike "_*" } | % { Write-Host "Importing function $_..."; Export-ModuleMember -Function $_ } 
    ...
    $includedAliases | ? { $_ -notlike "_*" } |  % { Write-Host "Importing alias $_...";  Export-ModuleMember -Alias $_ }
}