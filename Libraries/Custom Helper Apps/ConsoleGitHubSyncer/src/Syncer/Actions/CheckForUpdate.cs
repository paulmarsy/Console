namespace ConsoleGitHubSyncer.Syncer.Actions
{
    ...
    public partial class Synchronizer
    {
        ...
        {
            if (Git.InvokeWithOutput((Command) new[] {"rev-list HEAD..{0}@{{upstream}} --count", _currentBranch}).StandardOutput.Select(x =>
            {
                int diff;
                int.TryParse(x, out diff);
                return diff;
            }).Single() > 0)
                return true;

            if (Git.InvokeWithOutput((SubmoduleCommand)new[] { "rev-list HEAD..{0}@{{upstream}} --count", _currentBranch }).StandardOutput.Select(x =>
            {
                int diff;
                int.TryParse(x, out diff);
                return diff;
