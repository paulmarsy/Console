Invoke-InstallStep "Configuring Console Visual Studio Integration" {
	...
	New-ItemProperty "HKCU:\Software\Microsoft\VisualStudio\12.0\DialogPage\GrzegorzKozub.VisualStudioExtensions.ConEmuLauncher.Options" "Path" -Value "$conEmuExecutable" -Type String -Force | Out-Null
	New-ItemProperty "HKCU:\Software\Microsoft\VisualStudio\12.0\DialogPage\GrzegorzKozub.VisualStudioExtensions.ConEmuLauncher.Options" "DefaultWorkingDirectory" -Value "%HOMEDRIVE%%HOMEPATH%" -Type String -Force | Out-Null
	...
	New-ItemProperty "HKCU:\Software\Microsoft\VisualStudio\12.0\DialogPage\GrzegorzKozub.VisualStudioExtensions.ConEmuLauncher.Options" "TaskToExecute" -Value "" -Type String -Force | Out-Null
}