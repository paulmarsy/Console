...
$includeFile = Join-Path $ProfileConfig.General.PowerShellUserScriptsFolder "include.ps1"
if ((Test-Path $includeFile) -and -not ([String]::IsNullOrWhiteSpace([IO.File]::ReadAllText($includeFile)))) {
    ...
    try {
        Push-Location $ProfileConfig.General.PowerShellUserScriptsFolder
        . $includeFile
    }
    finally {
        Pop-Location
    ...
    $includedFunctions | ? { $_ -notlike $ExportExclusionPattern -and (Get-Command $_).ModuleName -eq $ExecutionContext.SessionState.Module.Name } | % { Write-Host "Importing function $_..."; Export-ModuleMember -Function $_ } 
    ...
    $includedAliases | ? { $_ -notlike $ExportExclusionPattern -and (Get-Command $_).ModuleName -eq $ExecutionContext.SessionState.Module.Name } |  % { Write-Host "Importing alias $_...";  Export-ModuleMember -Alias $_ }
